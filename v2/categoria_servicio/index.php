            ...
            case 'PUT':
                if ($_header == $_token_put) {
                  ...
                  if(strlen($body->nombre)>0){
                        $nombre = $control->putNombreById($body->nombre, $body->id);
                        }
                        if(strlen($body->imagen)>0 ){
                        $imagen = $control->putImagenById($body->imagen, $body->id);
                        } 
                        if(strlen($body->texto)>0 ){
                            $texto = $control->putTextoById($body->texto, $body->id);
                            } 



                    http_response_code(200);
                    echo json_encode(["data" => $texto]);
